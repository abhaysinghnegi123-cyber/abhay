...
const FIREBASE_RTD_PATH = "mcq_app/shared_data";

const FIREBASE_CONFIG = {
  apiKey: "AIzaSyBjRbpQAzwPIkcKU90l9cNSKmMtVOinmRc",
  authDomain: "mcq-f4c19.firebaseapp.com",
  projectId: "mcq-f4c19",
  storageBucket: "mcq-f4c19.firebasestorage.app",
  messagingSenderId: "480673548198",
  appId: "1:480673548198:web:89e851099910e51e303167",
  databaseURL: "https://mcq-f4c19-default-rtdb.asia-southeast1.firebasedatabase.app/"
};
...
function normalizeQuestion(question) {
  ...
  const correct = typeof question.correct === "string" ? question.correct.toUpperCase() : "A";
...
function normalizeSlots(input) {
  ...
  return rawSlots
    ...
    .map((slot) => ({
      ...
      questions: Array.isArray(slot.questions) ? slot.questions.map(normalizeQuestion) : []
    }))
    .filter((slot) => slot.code && slot.name);
}
...
function handleCloudUpdate(nextSlots) {
  slotsCache = normalizeSlots(nextSlots);
...
async function initDataStore() {
  ...
  try {
    ...
    const dataRef = rtdb.ref(FIREBASE_RTD_PATH);
    const snapshot = await dataRef.once("value");
    const remote = snapshot.val();
    if (remote && remote.slots) {
      slotsCache = normalizeSlots(remote.slots);
    ...
    unsubscribeCloud = dataRef.on("value", (snap) => {
      const value = snap.val();
      if (!value) return;
      const remoteSlots = value.slots;
...
function getSlots() {
  return cloneSlots(normalizeSlots(slotsCache));
}

function saveSlots(slots) {
  slotsCache = normalizeSlots(slots);
  localStorage.setItem(STORAGE_KEY, JSON.stringify(slotsCache));

  if (!cloudEnabled || !rtdb) return;

  rtdb
    .ref(FIREBASE_RTD_PATH)
    .set({
      slots: slotsCache,
      updatedAt: Date.now()
    })
...
function setTeacherView() {
  ...
  renderTeacherSlots();
  showTeacherModule("teacherCreateCard");
}
...
function setStudentView() {
  ...
  renderStudentSlots();
  showStudentModule("studentSlotCard");
}
